object EnumTask:
  ...
  def isTreeBalanced[A](tree: Tree[A]): Boolean =
    def helper[A](tree: Tree[A]): (Int, Boolean) =
      tree match
        case Stump => (0, true)
        case Leaf(_) => (1, true)
        case Branch(l, _, r) =>
          val (lHeight, lBalanced) = helper(l)
          val (rHeight, rBalanced) = helper(r)
          val treeHeight = 1 + Math.max(lHeight, rHeight)
          val treeBalanced = lBalanced && rBalanced && Math.abs(lHeight - rHeight) <= 1
          (treeHeight, treeBalanced)
    helper(tree)._2
