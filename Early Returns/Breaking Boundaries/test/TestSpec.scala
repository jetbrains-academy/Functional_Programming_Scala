...
class TaskSpec extends AnyFunSuite {
  ...
  test("Find First Valid Cat returns the valid cat and doesn't traverse the whole collection. Validation is only run when a cat is in the database and has not been adopted") {
    runTest(13, "Validation of fur characteristics", findFirstValidCat)
  }
}
